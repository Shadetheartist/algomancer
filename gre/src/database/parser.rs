...
use crate::card_type::{CardType, MetaCardType};
use crate::cost::Cost;
...
pub fn parse_json(raw_json: &str) -> Result<HashMap<PaperCardId, PaperCard>, serde_json::Error> {
    let data: HashMap<String, Vec<RawCardData>> = serde_json::from_str(raw_json)?;

    let mut c = 0;
    let mapped: Vec<PaperCard> = data.into_values().map(|mut d| {
        let d = d.remove(0);
        c += 1;

        PaperCard {
            id: PaperCardId(d.name.clone()),
